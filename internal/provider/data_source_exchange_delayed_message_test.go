...
func TestAccExchangeDelayedMessage_DataSource(t *testing.T) {
	if os.Getenv("TF_ACC") == "" {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					acceptance_test.That("data."+data.ResourceName).Key("delayed_type").HasValue(r.DelayedType),
...
func TestAccExchangeDelayedMessage_DataSourceNotExist(t *testing.T) {
	data := acceptance_test.BuildTestData("rabbitmq_exchange_delayed_message", "test")
	r := acceptance_test.ExchangeDelayedMessageResource{
		ExchangeResource: acceptance_test.ExchangeResource{
			Name: data.RandomString(),
